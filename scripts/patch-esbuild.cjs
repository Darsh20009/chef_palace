...
// Vite 5 uses esbuild 0.21.5 internally. The Replit env has 0.28.0 globally,
// so we place a 0.21.5 binary inside vite's nested node_modules so it wins
...
const TARBALL_URL = 'http://package-firewall.replit.local/npm/@esbuild/linux-x64/-/linux-x64-0.21.5.tgz';
...
function getOrDownloadBinary() {
  ...
  spawnSync('curl', ['-s', '-o', tmpTar, TARBALL_URL]);
  spawnSync('rm', ['-rf', tmpDir]);
  fs.mkdirSync(tmpDir, { recursive: true });
  spawnSync('tar', ['-xzf', tmpTar, '-C', tmpDir]);
